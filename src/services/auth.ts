...
async function sendSignupVerification(newUser: User, emailOtp: string): Promise<void> {
...
export function getDemoPasswordForEmail(email: string): string | undefined {
  return DEMO_ACCOUNT_PASSWORDS[email.trim().toLowerCase()];
}

export function isDeveloperAdminCredentials(email: string, password: string): boolean {
...
export function getVerificationDestination(user: Pick<User, 'email' | 'phone'>, type: 'email' | 'phone'): string {
  if (type === 'email') {
    return user.email;
  }
...
async function deliverVerificationCode(type: 'email' | 'phone', destination: string, code: string): Promise<void> {
  if (type === 'email') {
...
async function ensureDemoUser(email: string, password: string): Promise<User | undefined> {
...
export class AuthService {
  ...
  async sendVerification(userId: string, type: 'email' | 'phone'): Promise<{ type: 'email' | 'phone'; message: string }> {
  ...
  async verifyCode(userId: string, type: 'email' | 'phone', code: string): Promise<User> {
    const user = await db.getUser(userId);
    if (!user) {
      throw new Error('User not found');
    }
