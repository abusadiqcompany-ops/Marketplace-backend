...
const removeChatSubscriber = (chatId, subscriberId) => {
...
app.post('/api/payments/paystack/webhook', express.raw({ type: '*/*', limit: '1mb' }), async (req, res) => {
    try {
...
app.post('/api/payments/flutterwave/webhook', express.raw({ type: '*/*', limit: '1mb' }), async (req, res) => {
    try {
...
app.post('/api/orders/:id/fulfillment', verifyAuthToken, asyncHandler(async (req, res) => {
...
// ============== CHAT ==============
app.get('/api/chat/:chatId/messages', asyncHandler(async (req, res) => {
...
app.get('/api/chat/stream', (req, res) => {
    ...
    res.flushHeaders?.();
    const subscriber = { id: uuidv4(), res };
    const subscribers = chatSubscribers.get(chatId) || new Set();
